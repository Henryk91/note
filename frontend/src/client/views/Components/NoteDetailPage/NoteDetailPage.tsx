...
const NoteDetailPage: React.FC<NoteDetailPageProps> = ({ match, searchTerm, set }) => {
  const { noteNames, showAddItem, editName, pages } = useSelector((state: RootState) => state.person);
  ...
  const pagesCont =
    localPages && noteNames
      ? localPages.map((pageLink, index) => {
